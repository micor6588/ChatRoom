...
func (pro *SmsProcess) SendPrivateMessage(userID int, content string) (err error) {
	//创建一个mes
	var mes message.Message
	mes.MessageType = message.SmsPrivateMessageType
	//2.创建一个SmsMessage实例
	var smsMes message.SmsPrivateMessage
	smsMes.SmsMessage.Content = content
	smsMes.SmsMessage.UserID = CurrentUser.UserID
	smsMes.UserStatus = CurrentUser.UserStatus
	smsMes.ChatUserID = userID
	//3.序列化smsMes
	data, err := json.Marshal(smsMes)
	if err != nil {
		fmt.Println("smsMes json.Marshal faild err=", err)
